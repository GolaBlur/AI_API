...
class deepFake_execute:
    def image(target_entity, source_entity):
        ## s3에서 처리할 파일 다운로드
        target_file = bring_one_object(object_entity=target_entity)
        ...
        ## 기능 수행
        res_file_path = deepFake_image_func(source_img_path=target_file_path, target_img_path=source_file_path, use_gpu=True)
        
        if res_file_path is None:
            print("No res_file")
            return None
        
        res_file = open(res_file_path, 'rb')
    ...
    def video(target_entity, source_entity):
        ...
        ## 기능 수행
        res_file_path = deepFake_video_func(target_video_path=target_file_path, source_img_path=source_file_path, use_gpu=True)
    ...
    def group_images(source_group_entity_list, target_entity):
        ...
        source_file_path = []
        for source_entity in source_group_entity_list:
            source_file = bring.bring_file_from_s3(file_entity=source_entity)
            source_file_path.append(source_file.name)
        
        ## TODO 이미지 그룹 딥페이크 기능 함수 구현 후 여기서 호출하여 기능 수행
        ## 하나의 이미지에서의 얼굴을 모든 그룹 이미지에 적용시킴..
        res_file_list = []
        for source_file_path in source_file_path:
            res_file_path = deepFake_image_func(target_img_path=source_file_path, source_img_path=target_file_path, use_gpu=True)
            if res_file_path is None:
                res_file_list.append(None)
                continue
            res_file = open(res_file_path, 'rb')
        ...
        for i in range(len(source_group_entity_list)):
            res_file_entity = change_file_to_entity_and_store_at_s3(file=res_file_list[i], original_file_entity=source_group_entity_list[i])
            res_file_entity_list.append(res_file_entity)
        
        print("group images deepFake success")
...
def image_func_test():
    ...
    res = deepFake_image_func(target_img_path= target_file_path, source_img_path= source_file_path, use_gpu=True)
        
    return

def video_func_test():
    
    source_file_path = 'D:/ImmersionProject/FinalProject/GolaBlur/API-AI/AI_API/deepFake/data/target_img.jpg'
    target_file_path = 'D:/ImmersionProject/FinalProject/GolaBlur/API-AI/AI_API/deepFake/data/source_video.mp4'
